...
- 0.9 は後方互換性がないため、模擬的に 1.0 を使用する
...
- HTTP/0.9 はとてもシンプルで以下のことができなかった
 - 一つのドキュメントを送る機能しかない
 - 通信されるすべての内容は HTML であるという想定のため、ダウンロードするコンテンツのフォーマットをサーバーから返す手段がなかった
 - クライアント側から検索のリクエスト以外のリクエストが送れなかった
 - 文章の送信、更新、削除ができない
- HTTP/1.0 が2年後に公開
- リクエストに以下の変更がされた
 - GET メソッドが追加された
 - リクエスト時に HTTP バージョンが追加された
